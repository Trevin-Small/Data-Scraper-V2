...
def getAgencies(user, titleAgencies, windows, selenBrowser):
    ...
    wait()
    username.send_keys(user.get_username())
    wait()
    password = selenBrowser.find_element_by_id("password")
    wait()
    password.send_keys(user.get_password())
    wait()
    submit = selenBrowser.find_element_by_class_name("Submit")
    wait()
    submit.click()
    wait()
    ...
    openMta(windows, selenBrowser)
    windows.switchWindowByDict('driggsAgentList')
 
def openMta(windows, selenBrowser):
    ...
    wait(3)
    selenBrowser.find_element_by_name("user").send_keys("LornaPhillipy")
    wait()
    selenBrowser.find_element_by_name("pw").send_keys("$3scotlanD6")
    wait()
